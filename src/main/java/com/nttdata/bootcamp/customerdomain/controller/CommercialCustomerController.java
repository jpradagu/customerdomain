...
import com.nttdata.bootcamp.customerdomain.service.ComercialCustomerService;
...
public class CommercialCustomerController {
    ...
    @Autowired
    private ComercialCustomerService commercialService;

    @GetMapping
    public Mono<ResponseEntity<Flux<CommercialCustomer>>> findAll() {
        log.info("CommercialCustomerController findAll ->");
        return Mono.just(ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(commercialService.findAll()));
    }
    ...
    @PostMapping
    public Mono<ResponseEntity<Map<String, Object>>> create(@Valid @RequestBody Mono<CommercialCustomer> monoCommercial) {
        log.info("CommercialCustomerController create ->");
        Map<String, Object> result = new HashMap<>();
        return monoCommercial.flatMap(c -> {
            c.setId(null);
            return commercialService.create(c).map(p -> ResponseEntity
    ...
    @PutMapping("/{id}")
    public Mono<ResponseEntity<Map<String, Object>>> update(@Valid @RequestBody Mono<CommercialCustomer> monoCommercial, @PathVariable String id) {
        log.info("CommercialCustomerController update ->");
        Map<String, Object> result = new HashMap<>();
        return monoCommercial.flatMap(c -> commercialService.update(c, id)
                .flatMap(enterprise -> {
    ...
    public Mono<ResponseEntity<Void>> delete(@PathVariable String id) {
        ...
        return commercialService.findById(id).flatMap(e -> commercialService.delete(e).then(Mono.just(new ResponseEntity<Void>(HttpStatus.NO_CONTENT))))
