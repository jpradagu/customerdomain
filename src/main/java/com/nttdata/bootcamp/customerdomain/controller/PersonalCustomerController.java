...
public class PersonalCustomerController {
	...
	public Mono<ResponseEntity<Map<String, Object>>> create(@Valid @RequestBody Mono<PersonalCustomer> monoCustomer) {
		...
		return monoCustomer.flatMap(c -> {
			...
			return personalService.save(c)
					.map(personal -> ResponseEntity
							.created(URI.create("/api/customer/personal/".concat(personal.getId())))
	...
	@PutMapping("/{id}")
	public Mono<ResponseEntity<PersonalCustomer>> update(@RequestBody PersonalCustomer personal,
	...
	public Mono<ResponseEntity<Void>> eliminar(@PathVariable String id) {
		return personalService.findById(id)
				.flatMap(
						p -> personalService.delete(p).then(Mono.just(new ResponseEntity<Void>(HttpStatus.NO_CONTENT))))
