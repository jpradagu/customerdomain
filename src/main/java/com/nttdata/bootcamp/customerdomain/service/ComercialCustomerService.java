...
public class ComercialCustomerService {
	...
	public Mono<CommercialCustomer> create(CommercialCustomer customer) {
		...
		return enterpriseRepository.findByRuc(customer.getRuc())
				...
				.switchIfEmpty(Mono.defer(() -> enterpriseRepository.save(customer))).cast(CommercialCustomer.class);
	}
		...
		return enterpriseRepository.findById(id)
				...
				.flatMap(p -> enterpriseRepository.findByRuc(commercialCustomer.getRuc())
						.switchIfEmpty(Mono.defer(
								() -> enterpriseRepository.save(parseIdToCommercialCustomer(commercialCustomer, id))))
						.flatMap(obj -> {
							if (obj != null) {
